pub fn generate_adsr_envelope(size: usize, ta: f32, td: f32, ts: f32, tr: f32) -> Vec<f32> {
...
pub fn adsr_multiplication(
    ...
    let endpose: usize = (startpose + size) as usize;
    for n in startpose..endpose {
        in_audio[n - startpose] *= adsr_modificator[n];
    }
    for n in size..frame_size {
        in_audio[n] = 0.0;
    }
}
